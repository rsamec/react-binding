...
var testSuite = (Binder: BindTo.BinderStatic) => {
    ...
    describe('binding to arrays', () => {
        ...
        it('default array', function () {
            ...
            var data = {
                Data: {
                    "People": Array.apply(null, Array(ARRAY_SIZE)).map(function (value, index) {
                        var odd = index % 2 !== 0;
                        return {
                            "Person": odd ? {
                                "FirstName": initValues.firstName,
            ...
            //exec
            var r = Binder.bindTo(data, "Data");
            var root = Binder.bindArrayTo(r, "People");
            ...
            for (let i = 0; i != ARRAY_SIZE; i++) {
                ...
                //verify
                execAndVerifyPersonProperties(nestedBindings, odd ? initValues : {}, changedValues);
            }
        });
